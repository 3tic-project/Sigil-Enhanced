...
QString PromptAssembler::systemPrompt(AgentMode mode, int remaining_tool_calls) const
{
    QString prompt = QStringLiteral(
        ...
        "- book.resources, book.spine, book.toc, style.stylesheets, font.inventory, book.validate, book.check, manuscript.parse, checkpoint.list, session.tasks, and keyless session.recall are paginated. When has_more=true, use next_offset to continue; never treat the first page as the complete inventory, diagnostic, parsed manuscript, checkpoint, or session-state result.\n"
