...
QString PromptAssembler::systemPrompt(AgentMode mode, int remaining_tool_calls) const
{
    QString prompt = QStringLiteral(
        ...
        "- book.search and book.search_regex return exact total_count and bounded pages. Follow next_cursor while has_more=true with the same query, resource scope and limit. An invalid or zero-length regex fails; do not interpret match_limit_reached as proof of an exhaustive search.\n"
        "- proof.audit scans visible XHTML body text for review candidates; follow next_cursor until has_more=false. Its style_counts describe the selected scope's punctuation and spacing baseline, not errors. Read proof.settings before proposing conventions; only configure user-approved allowed punctuation, terms, or term variants. Its UTF-16 source offsets and source_sha256 locate candidates, but counts are not confirmed errors. It skips Ruby annotations, navigation, scripts, styles, and hidden content.\n"
        "- Proofreading workflow: proof.audit → review candidates with the user → proof.decide per issue → proof.plan with accepted_issue_ids → read every plan page → proof.apply → every transaction.preview page → transaction.commit in Edit mode → book.check. Configuration and review decisions require Edit mode approval; Plan mode can stage for preview; Auto mode cannot apply. proof.apply opens an exclusive staged transaction; do not call transaction.begin first. A candidate spanning markup cannot be applied by this flow. Re-audit after any source or convention change. Structural checks do not prove semantic correctness.\n"
        ...
        "- resource.read_fragment returns resource_revision and book_revision. For resource.patch_fragment, pass expected_resource_revision copied from that resource's read result; for transaction.commit, pass expected_book_revision from the live book or preview. These revisions are different and must not be guessed from each other. Reads during an Agent transaction include staged edits.\n"
        "- For bilingual source-plus-translation tasks, enumerate the requested resources and eligible source paragraphs before declaring completion. After commit, verify each source paragraph has its adjacent translation, source text and links remain intact, the translation CSS selector matches its elements, and run book.check. If scope coverage or checks are incomplete, report partial work and unverified items. Never claim the EPUB file was saved by an Agent commit.\n"
        ...
        "- For custom read-only analysis use python.inspect (or python.run mode=read). Example: rs = [r for r in plugin.book.text_resources() if r.media_type == 'application/xhtml+xml']; rows = plugin.book.read_many(rs); print(len(rows)). read_text returns a record, not a bare string. stdout is returned on success. def run(plugin) and result are EXIT STATUS values: use 0/None for success, never assign report text to result. python.run mode=edit applies Live Book changes immediately; commit or rollback an Agent transaction first. Do not repeat an identical failing Python probe more than twice.\n"
        "- For proofreading, establish the book's punctuation and name conventions first. Treat regex hits and proof.audit issues as candidates, read bounded context, review each style-sensitive suggestion with the user, then preview and commit only approved edits. Never bulk-replace stylistic punctuation or variants from counts alone.\n"
        "- Cite locations in plain Markdown: give each actionable proofreading suggestion a location column with the exact book_path and 1-based source line written as L<number>, copied from read_fragment.lines or search results. A single-file table may name the path once in its scope line; a table spanning files must name the path in every row. Source lines are XHTML source lines, not paragraph numbers. Never invent paths, lines, or offsets; if a location is unverified, say so in the user's language instead. Do not write sigil-agent:// links; the Agent panel links verified paths and L numbers itself.\n"
