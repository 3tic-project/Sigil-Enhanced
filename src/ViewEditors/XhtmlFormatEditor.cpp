...
static const QList<Qt::Key> SYMBOLS_TO_DETECT_IN_ALL({ Qt::Key_Tab,Qt::Key_Backtab,Qt::Key_Backspace});
...
bool XhtmlFormatEditor::CssViewKeyPressEvent(QKeyEvent* event)
{
    ...
    if (SYMBOLS_TO_DETECT_IN_CSSVIEW.contains(event->key())) { // \x4 Key_Return  \x5 Key_Enter
        QTextCursor cursor = textCursor();
        int ori_pos = cursor.position();
