...
bool CodeViewEditor::HtmlViewKeyPressEvent(QKeyEvent* event)
{
    ...
    if (SYMBOLS_TO_DETECT_IN_HTMLVIEW.contains(event->key())) {
        QTextCursor cursor = textCursor();
        int ori_pos = cursor.position();
...
void CodeViewEditor::CommonPasteEvent(const QMimeData* source) {
    ...
    if (i > 0 && src_txt.at(i - 1) == QChar('\n')) {
        ...
        QString paste_txt = Utility::UseNFC(source->text());
    ...
    if (source->hasText() || source->hasHtml()) {
        QMimeData normalized;
        if (source->hasText()) {
            normalized.setText(Utility::UseNFC(source->text()));
        }
        if (source->hasHtml()) {
            normalized.setHtml(Utility::UseNFC(source->html()));
        }
        QPlainTextEdit::insertFromMimeData(&normalized);
    } else {
