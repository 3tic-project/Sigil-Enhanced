...
set( RAW_SOURCES ${MAIN_FILES} ${TAB_FILES} ${SOURCEUPDATE_FILES} ${BOOK_MANIPULATION_FILES} ${BUILTIN_PLUGIN_FILES} ${CHINESE_CONVERSION_FILES} ${RESOURCE_OBJECT_FILES} ${DIALOG_FILES} ${WIDGET_FILES} ${FONT_PREVIEW_FILES} ${EXPORTER_FILES} ${IMPORTER_FILES} ${MISC_FILES} ${MISC_EDITORS_FILES} ${QUERY_FILES} ${PARSERS_FILES} ${EMBEDPYTHON_FILES} ${SPCRE_FILES} ${VIEW_EDITOR_FILES} ${MAINUI_FILES} ${AGENT_FILES} )
...
source_group( "Font Preview"      FILES ${FONT_PREVIEW_FILES} )
...
source_group( "Agent"             FILES ${AGENT_FILES} )
...
set( LIBS_TO_LINK ${HUNSPELL_LIBRARIES} ${PCRE2_LIBRARIES} ${GUMBO_LIBRARIES} ${LEXBOR_LIBRARIES} ${MINIZIP_LIBRARIES} OpenCC::OpenCC Sigil::HarfBuzzSubset
...
if( UNIX AND NOT APPLE )
    ...
    if( APPIMAGE_BUILD )
        install( TARGETS sigil DESTINATION ${CMAKE_INSTALL_BINDIR} )
    else()
    ...
    if( NOT USE_SYSTEM_LIBS OR NOT HUNSPELL_FOUND )
        if( APPIMAGE_BUILD )
            install( PROGRAMS ${HUNSPELL_NAME} DESTINATION ${CMAKE_INSTALL_LIBDIR} )
        else()
    ...
    if( APPIMAGE_BUILD )
        install( PROGRAMS ${GUMBO_NAME} DESTINATION ${CMAKE_INSTALL_LIBDIR} )
    else()
