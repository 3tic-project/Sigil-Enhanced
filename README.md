...
- **[Native Agent](docs/NativeAgent.md)**：主窗口停靠栏里的 EPUB 助手（Ask / Plan / Edit）。在偏好设置里选择 DeepSeek / OpenCode Go / OpenRouter，并从服务器拉取模型；支持 thinking/`reasoning_content`、工具批准、可撤销事务、会话导出，以及复用原生 DIV/CSS 引擎的[段落分析与计划](docs/AgentNativeParagraphTools.md)。不调用 MCP，也没有 shell。
