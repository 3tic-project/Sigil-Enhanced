...
# Sigil修改版
Sigil是目前主流的E非商业化EPUB制作工具，界面直观，功能丰富，配插件系统，可完成复杂的EPUB编辑工作。对于EPUB校验严格，规范明确，是目前同行软件中对于EPUB规范化做的最好的软件。但是Sigil的缺点也是比较明显，操作卡顿，是Sigil长期存在的一个问题，由于其对于批量文件操作的逻辑不合理，导致进行文件操作时，文件数量跟耗时的关系呈指数曲线，在处理一些合集类的书籍，文件数量动辄上千，这种情况下进行文件批量操作往往要卡顿十几分钟到半小时，这点不少人有过切身体会。此外Sigil还有一些不够人性化的操作模式也是令人头疼。Sigil修改版的出现，正是为了优化Sigil的运行效率，提供一些更加人性化的操作模式，顺便解决一些遇到的BUG。

# 支持平台
仅支持`WIN10 (1809版以上)`，`WIN11`，`MacOS 11.X ~ MacOS 13.X`\
...
注意：Emmet生成代码格式绑定【XHTML自定义格式化】功能，如果自定义格式化设置不当，可能导致Emmet生成代码的格式不合理。
...
13. 添加【分割段落或插入br空行】功能——绑定快捷键Ctrl+Return\
...
2. 添加添加【Epub2转EPUB3】、【EPUB3转EPUB2】的功能\
