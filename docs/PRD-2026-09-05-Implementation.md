...
- 代码视图句子选择：`feature/code-view-sentence-selection`，从已验证的正文单元选择与
  Native Agent 有界化分支栈继续，补齐 PRD 中允许后置的 sentence 模式。
...
## 代码视图当前句子选择（2026-09-16）

分支：`feature/code-view-sentence-selection`。主要提交：`d9ce9950a`（源码安全的句界算法与
性能回归）、`64d91b40e`（编辑器、设置页与真实鼠标集成）、`a8399787e`（简中、繁中、日文
文案）。

### 从可见句子安全映射回 XHTML

- “偏好设置 → 编辑器 → 代码视图 → 双击选择”新增“当前句子”。既有默认值仍是
  `element-content`，旧的 `word` 设置和无法识别设置的归一规则不变；sentence 只在 XHTML、
  无 Shift/Alt 和无 Ctrl/Command 导航修饰键时启用。
- 策略层把正文单元投影成可见文字，排除标签，解码合法数字实体以及 XML/常见排版命名实体，
  再以 Qt Unicode sentence boundary 识别中日韩标点、西文句点、尾随引号、小数和网址。每个
  投影字符保留原始 UTF-16 范围，因此结果仍使用原实体拼写并可从 Ruby 基字或注音映射到同一句。
- 完整落入句子的行内元素会连同成对标签扩入选区；若映射结果只包含一侧标签，则回退到当前
  正文单元。属性、注释、CDATA、畸形结构、非 XHTML 编辑器，以及含 code/pre/script/style、
  SVG 或 MathML 后代的正文单元继续使用原有词选择。选择只移动 `QTextCursor`，不改变文档、
  modified 状态或 undo 栈。
- 每次只投影点击点左右各 65,536 个源码 UTF-16 单元。句界触及人工窗口而正文仍向外延伸时，
  返回 `NoSentence` 并走词选择，避免异常超长单句造成无界延迟或猜测截断。内联标签展开使用
  单调源码映射的二分查找，不再对每个标签线性扫描整段投影。

### 测试证据与剩余项

`code_view_selection_policy` 覆盖中文/日文、Ruby、数字和命名实体、小数、网址、尾随引号、
完整与部分行内包装、不平衡及危险标签、20,000 标签暖索引、2,000 个行内标签、超长单句回退
和 1 MiB 正文末尾句子；暖索引 P95 预算 50 ms，重型句子路径预算 100 ms。
`code_view_selection_integration` 链接真实编辑器与偏好设置对象，验证实际双击坐标、三种设置值、
Ruby/中日韩句子、小数、网址、属性/修饰键/非 XHTML 回退，以及选择不污染文档和 undo 栈。
完整 Sigil 构建与 42 项固定 Python 依赖检查通过；四语 `.qm` 目标与简中/繁中/日文严格目录
覆盖通过，三种翻译目录各 5,753 条且 0 unfinished。

当前句界依赖 Qt/ICU 数据，不提供自然语言语义分句保证；未知命名实体只作为不可拆分占位符，
...
## Clips 本机键盘与主题复核（2026-09-19）

对 C08–C10 增加真实工具栏检查，发现第一至第十个 Clip 的 Qt 工具栏按钮默认为
`NoFocus`，虽有可访问名称却无法进入 Tab 顺序。现仅让这十个按钮取得 `StrongFocus`；
角标层仍不可聚焦、不截获鼠标。集成测试通过 Qt 可访问接口直接核对完整名称、焦点
状态、Clip 1→2 的实际 Tab 导航，以及快捷键自定义/清空后可访问名称的实时更新。
macOS 默认 Clip 1 验证为原生 `⌥⌘1` 表示。

角标绘制改为读取所属按钮的实时调色板，避免切换高对比度调色板后仍显示旧蓝色。
100%、125%、150%、200% 缩放的角标测试通过；浅色、模拟深色和模拟高对比度的
工具栏截图已检查，后两者还自动校验角标区域确实采用当前高亮色。真实操作系统主题、
VoiceOver 和 Windows/Linux 仍未验收，故 G7 继续保留为未关闭。

## Live 事务强杀恢复验证（2026-09-20）

新增 macOS 原生集成用例，在必需的恢复检查点写好之后、Live 事务第一个资源已经
真实落盘而后续正文尚未应用时，向宿主发送 SIGKILL。测试不是普通异常回滚：杀掉
进程后由另一进程读取仓库标签并走现有 `repomanager.generate_epub_from_tag` 恢复路径。
它核对原 EPUB 哈希不变、恢复物包含提交前尚未保存的正文、资源集合完整且不含已
落盘的半提交 CSS。强杀用例和原有多阶段进程内故障补偿用例各连续执行 3 次，均通过。

为精确触发物理变更边界，仅在原有 `PluginSessionManager` 测试故障钩子上增加可选
回调；正常运行不设置该回调。验证范围是 Live 多资源提交的此一强杀边界，不等于
电源中断下文件系统与仓库对象的落盘持久性，也不代表 DIV、TOC、直接 Save 等所有
入口的崩溃矩阵或实际恢复对话框已通过。因此 G6 继续保留“部分”。
