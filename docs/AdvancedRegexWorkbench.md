...
工作台是独立的非模态顶层窗口，可从任务栏/窗口切换器单独选中；打开期间仍可回到 Sigil 正文编辑器查找、复制或核对句子。再次执行菜单命令会聚焦已有工作台，不会重复创建窗口。标题会显示“高级正则工作台 — 当前 EPUB 文件名 / EPUB 版本 / Sigil”，并同步主窗口的修改状态。
...
4. 点击突出显示的 **Apply**，确认目标资源数。工作台会重新抓取书籍快照并从头运行，不会复用可能过期的 Dry Run 结果。
...
切回工作台或开始运行时，“当前文件”和“选中文件”会从 Sigil 重新读取，因此可以先在正文中定位章节，再回到工作台处理。若在运行期间继续修改目标正文，Apply 的提交冲突检查会拒绝旧快照结果；Dry Run 报告仍可查看，但双击旧结果只跳到邻近行，不会按失效坐标高亮。切换 EPUB 会取消并关闭属于旧书籍的工作台。
...
可选择当前文件、Book Browser 中选中的文本文件、全部 XHTML、全部 CSS、全部特殊文本文件或全部文本资源。“特殊文本文件”包括 OPF、NCX、SVG、XML 和 MiscText；EPUB 3 Nav 是 XHTML，因此归入 XHTML 作用域。“全部文本资源”也包含上述特殊文件。工作台只在 GUI 线程抓取资源快照和最终提交；实际正则计算只接触内存中的 `QString`。OPF/NCX 等 XML 文件同样经过暂存 XML 校验后才允许提交。

变量列表位于右侧运行区下方；区域内标题“Variables”在左，“Clear Variables”在右。运行后可直接检查 Scope、Resource、Name 和 Value；清空按钮只清除工作台会话变量。Dry Run 结果表逐条显示规则、文件、行号、替换前后片段、递归轮次和发生变化的变量名。双击结果会打开对应资源、跳到快照中的原文位置，并在坐标仍可精确映射时选中匹配范围。若某次匹配依赖前一轮生成的文本，或报告生成后正文再次变化，工作台会退化为跳到最接近的原文行而不误选文本。Apply 成功后的结果使用最终文档坐标；被后续规则覆盖的范围同样只定位到邻近位置。

明细行和片段有硬上限，超过后表格会提示省略行数，但总匹配数和总替换数仍保持准确。

## Recipe 文件

Recipe 使用版本化 JSON 格式，默认目录为 Sigil 首选项目录下的 `regex_workbench/`。工作台支持新建、打开和原子保存。当当前方案不再是未经编辑的全新状态时，点击“New”会先提示现有规则和未保存更改将被清空。规则编辑区使用可拖动的三栏布局，并把匹配表达式、规则选项和命名捕获分组显示。

“导入搜索模板”入口当前暂时隐藏；兼容转换代码仍保留，待预览型模板的批量应用语义完成审计后再开放。已有 Recipe 和 Automate 调用不受影响。
...
- 旧版“日文引号纠正”模板把普通空格误写进 `[「 」]` 字符类，可能匹配并破坏标签属性空格；加载旧模板时会将其修正为 `[「」]`，Replacement 同时修正为 `「\1」`。只有名称、Find 和 Replacement 均与旧内置模板完全一致时才迁移，不改动用户自定义版本。
